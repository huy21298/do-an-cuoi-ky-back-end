...
const loadBaiThi = (req, res) => {
    ...
    //console.log(_id);
     ...
     BaiThi.findOne({ _id }).where("trang_thai", true)   /** cái này dùng đễ load bài thi thử */
    ...
    .then( (baiThi) => {
        if (!baiThi){
            return res.status(status.INVALID_FIELD).json({   
                ...
                errors: [
                  {
                    msg: "Bài thi không tồn tại hoặc đã được thi rồi",
                    param: "_id",
                  },
                ],
              }); 
            ...
            return res.status(status.SUCCESS).json({data: data, success: true})
         }
    })
