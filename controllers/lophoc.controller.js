...
const loadBaiTapTrongMotLop = (req, res) => {
  ...
  BaiTap.find({ lop_hoc_id }).select("tieu_de han_nop_bai nguoi_tao_id noi_dung createdAt ")
        .populate({ path: "nguoi_tao_id", select: "ho ten" })
        .then(baiTap => {
          const data = {
            bai_tap: baiTap
...
const hanLamBai = async (req, res) => {
  try {
    ...
    if (countBaiThi > 0) {
      const baiThi = await BaiThi.findOne({ lop_hoc_id })
        .select("tieu_de ngay_thi")
        .sort({ ngay_thi: 01 > Date() })
        .limit(1);
      data.push(baiThi);
    } if (countBaiTap > 0) {
      const baiTap = await BaiTap.findOne({ lop_hoc_id })
